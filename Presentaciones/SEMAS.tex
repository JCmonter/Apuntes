...
\vspace{0.1cm}
\centering{\small\itshape La topología clásica estudia espacios como conjuntos de puntos organizados por abiertos.}
\end{frame}
...
\vspace{0.1cm}
\centering{\small\itshape La topología puede desarrollarse sin mencionar puntos explícitamente.}
\end{frame}
...
\vspace{0.1cm}
\centering{\small\itshape La topología sin puntos es hoy un área viva de investigación.}
\end{frame}
...
\begin{itemize}\justifying
    \item Cada vez más personas se interesan en aprender sobre esta área.
    \item En CUCEI se estudian axiomas de separación, construcciones de parches, .
\end{itemize}

\vspace{0.1cm}
\centering{\small\itshape ¿Consideras que en algún momento has realizado topología sin puntos?}
\end{frame}
...
Los axiomas de separación son nociones topológicas que permiten distinguir puntos distintos (o conjuntos cerrados)
a través de conjuntos abiertos.
...
\begin{frame}{¿Qué es un marco?}
  ...
  \onslide<5->\begin{block}{Marco}
    ...
    \]
  \end{block}
\onslide<6->{LDM: Ley Distributiva de marcos}
\end{frame}
...
\end{tikzcd}\]
  ...
  \vspace{0.5em}
  \uncover<6->{\centering\textcolor{blue!60!black}{\large ``La topología sin puntos y la topología clásica se conectan por esta adjunción''}}
\end{frame}
...
La topología sin puntos permite ``traducir'' nociones clásicas de la categoría de espacios topológicos al contexto de marcos, y viceversa.
  ...
  \uncover<3->{\centering
    \textcolor{blue!60!black}{\Large 
      ``¿Consideras que en algún momento has realizado topología sin puntos?''}
  }

  \vspace{0.5em}

  \uncover<4->{\centering
    \textbf{\Large Seguramente sí...}
  }
\end{frame}
...
\begin{tikzpicture}[>=latex, every node/.style={align=center}]
  ...
  \tikzset{
    box/.style={draw=black!30, rounded corners, fill=white,
                text=orange!70!black, font=\small, inner sep=4pt,
                minimum width=44mm, minimum height=10mm},
    lab/.style={font=\bfseries\footnotesize, text=black!60},
  ...
  \node[box] (t2) at ($(L)+(0,-1.0)$) {Reescribir la noción en términos\\ del marco de abiertos};
...
\begin{tikzpicture}[>=latex, every node/.style={align=center}]
  ...
  % cajas
  \node[box] (t1) at ($(L)+(0,1.2)$) {Definir caracterizaciones\\ para espacios topológicos};
  \node[box] (t2) at ($(L)+(0,-1.0)$) {Obtener una equivalencia\\ para el marco de abiertos};

  \node[box] (f1) at ($(R)+(0,1.2)$) {Noción en la teoría\\ de marcos};
...
\begin{frame}{Axiomas de separación en Top}
  ...
  \vspace{0.6em}
  \centering
  \textcolor{blue!60!black}{\large¿Cómo se leen estas ideas \emph{sin puntos}?}
\end{frame}
...
existen $U,V\in \mathcal{O}S$ con $A\subseteq U$, $B\subseteq V$ y $U\cap V=\varnothing$.
 ...
 \uncover<3->{\centering
  \textcolor{blue!60!black}{\large¿Qué necesitamos para hacer la traducción?}}
