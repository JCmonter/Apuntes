...
\end{itemize}
\onslide<7->{¿Qué pasa si el marco $A$ cumple $\mathbf{(H)}$?}
\end{frame}
...
\onslide<2->{Si $j\in NA$, $\hat{f}^\infty$ es el núcleo asociado al filtro $j_*F\in A^\wedge$.}
...
\onslide<2->{Definiendo $H=f^\infty\circ j$ y $h=H_{\mid A_{j_*F}}$, obtenemos el diagrama}
\onslide<3->\begin{equation}\label{diagrama h}
	\begin{tikzcd}
	A & {A_{\hat{f}^\infty}} \\
	{A_j} & {A_{f^\infty}}
	\arrow["{\hat{f}^\infty}", from=1-1, to=1-2]
