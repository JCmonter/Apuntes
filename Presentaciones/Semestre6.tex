...
\footnotetext[0]{Aspectos básicos sobre teoría de marcos y espacios topológicos pueden ser consultados en \cite{Johnstone82, PicadoPultr, picado2021separation, simmonsbasics, simmonscollection}}
\end{frame}

\begin{frame}{Derivadas y prenúcleos}
  Sea $A\in \Frm$
\[
j\colon A\to A,\quad j\in NA\subseteq PrA
\]
\onslide<2->{Considerando $S\in \Top$ y $\mathcal{O}S\in \Frm$
\[
  \partial\colon \mathcal{C}S\to \mathcal{C}S,\qquad f\colon \mathcal{O}S\to \mathcal{O}S.
\]
Si $f\in Pr\mathcal{O}S$, entonces
\[
f(U)'=\partial(U')
\]}
\onslide<3->{Ejemplos:
\[
[E](U)=(E\cup U)^{\circ},\; \partial_Q(X)=\overline{(Q\cap X)}=v_Q(X')',\; \partial_Q^\infty(X)=v_\nabla(X')'
\]
donde $Q\in \mathcal{Q}S$, $X\in \mathcal{C}S$, $U\in \mathcal{O}S$ y $\nabla\in \mathcal{O}S^\wedge$.}
\end{frame}
...
\[ 
f_F=\bigvee\{v_a\mid a\in F\}\in PrA.
\]
...
\begin{frame}{Propiedades de los marcos eficicientes}
  ...
  \begin{itemize}
    ...
    \item $S$ es apilado y empaquetado $\iff$ $\mathcal{O}S$ es eficiente.
  \end{itemize}
...
\begin{block}{Definición \cite[Def 4.1]{simmons2004vietoris}:}
...
\onslide<2->{\begin{block}{Teorema \cite[Teo 5.7]{simmons2004vietoris}:}
...
\onslide<3->{\begin{block}{Teorema \cite[Teo 7.2]{sexton2006ordinal}:}
...
\begin{frame}[fragile]
Considerando $(A^\wedge, \mathcal{O}A^\wedge)$, donde 
\[
\mathcal{O}A^\wedge=\langle D(x)=\{F\in A^\wedge\mid x\in F\}\rangle  ,
\]
tenemos
\[\begin{tikzcd}
	{\pt(VA)} && {A^\wedge} && {\mathcal{O}A^\wedge} && {\mathcal{O}\pt(VA)}
	\arrow["\pi", from=1-1, to=1-3]
	\arrow["{\mathcal{O}(\pi)=\pi^{-1}}", from=1-5, to=1-7]
\end{tikzcd}\]
