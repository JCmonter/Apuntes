...
\onslide<3->{Ejemplos:
\[
[E](U)=(E\cap U)^{\circ},\; \partial_Q(X)=\overline{(Q\cup X)}=v_Q(X')',\; \partial_Q^\infty(X)=v_\nabla(X')'
\]
...
Consideremos $S\in\Top$. Tenemos $Q\in \mathcal{Q}S$ y $\nabla\in \mathcal{O}S^\wedge$. Al par $(F, Q)$ lo llamamos para HM.
Adicionalmente
...
\begin{frame}
\begin{block}{Proposición:}
Sean $A\in\Frm$ y $(F.Q,\nabla)$  una terna HM. Para $j\in NA$ se cumple que:
\begin{enumerate}
  \item Si $A$ es fuertemente apilado, entonces $v_\nabla\circ j=j\circ v_F$.
  \item Si $A$ es eficiente y $v_\nabla\circ j=j\circ v_F$, entonces $A$ es fuertemente apilado,
\end{enumerate}
\end{block}
