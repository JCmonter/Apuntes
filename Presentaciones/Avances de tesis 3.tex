...
\onslide<6->{¿Cuál es el análogo de empaquetado en $\mathbf{Frm}$?}
\end{frame}
...
\begin{frame}
Sea $A\in \mathbf{Frm}$. Para $a, b\in A$, $j\colon A\to A$ es un \emph{núcleo} si:
\begin{enumerate}
...
\begin{frame}{El ensamble parches}
	...
	\uncover<2->{Sea $P(A)=\langle\text{p-base}(A)\rangle$, es decir, tomamos supremos arbitrarios de elementos en $\text{p-base}(A)$
	 ...
	 \[\xymatrix{ A\ar[r]^{i}\ar@/^/@<+1.5ex>[rr]^{\eta_{A}} & P(A)\ar[r]^{\iota} & NA }\]
	}
...
\begin{frame}{Regularidad implica parche trival}
	...
	\onslide<2->{\begin{proof}
		\begin{enumerate}
			...
			\item<4-> $A$ es ajustado $\Rightarrow$ cada bloque tiene un solo elemento.
			\item<5-> $x=\bigvee\{y\in A\mid (\exists \,z)[z\wedge y=0 \mbox{ y }z\vee x=1]\}$ es dirigido.
			\item<6-> $\nabla(u_d)=\nabla(j)\Rightarrow j\sim u_d$.
			\item<7-> $A$ es ajustado $\Rightarrow$ $u_d$ es el único en su bloque.
			\item<8-> $j=u_d$.
		\end{enumerate}
	\end{proof}}
\end{frame}    
