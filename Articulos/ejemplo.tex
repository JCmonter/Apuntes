...
\section{Algunas propiedades de separación libres de puntos adicionales}
En la literatura existen algunas propiedades de separación adicionales que, desde que fueron enunciadas, se presentaron sin la dependencia de los puntos. En este apartado mencionaremos las características principales de estas propiedades. Para consultar más información sobre ellas ver \cite{J.P.2}.\\

\subsection{El axioma $T_D$}
La primera que discutiremos (aunque aun tiene la presencia de puntos), es más fuerte que $T_0$, pero más débil que $T_1$.
...
A partir de este momento, comenzaremos a trabajar con cuestiones localicas, por ende, comenzaremos a usar términos como ``sublocal'', ``sublocal abierto'' y ``sublocal cerrado'' ($o(\_)$ y $c(\_)$, respectivamente), e indirectamente usaremos la correspondencia biyectiva que existe entre los sublocales y ciertos operadores definidos en los marcos conocidos como \emph{núcleos}. Para ver información sobre estos y la correspondencia biyectiva antes mencionada sugerimos consultar \cite{J.M.} o \cite{A.Z.}.  
